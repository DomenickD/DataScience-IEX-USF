...
table_option = st.radio("Select what dataset you would like to obtain a prediction from:", 
...
if table_option == "Titanic":
    ...
    if st.button("Predict"):
        response = requests.post("http://flask_route:5000/predict_titanic", json=user_input)
    ...
    TotRms_AbvGrd = st.number_input('Total rooms above grade:', min_value=0, max_value=5, value=2)
    ...
    if st.button("Predict"):
        response = requests.post("http://flask_route:5000/predict_housing", json=user_input)
        result = response.json()
    ...
    if st.button('Predict'):
        response = requests.post("http://flask_route:5000/predict_sentiment", json={"text": query})
        result = response.json()
    ...
    if st.button('Predict'):
        if canvas_result.image_data is not None:
            ...
            response = requests.post("http://flask_route:5000/predict_digit", json={"image_data": image_data})
            result = response.json()
