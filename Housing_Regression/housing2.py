...
df = pd.read_csv('Housing_Regression\AmesHousing.txt',
                 sep='\t',
                 usecols=columns)
...
def train_and_evaluate_model(pipeline, X_train, y_train, X_test, y_test, model_name):
    ...
    # Learning Curve
    train_sizes, train_scores, test_scores = learning_curve(pipeline, X_train, y_train, cv=5, scoring='r2', n_jobs=-1, train_sizes=np.linspace(0.1, 1.0, 10))
    ...
    param_range = np.arange(10, 200, 10) if model_name == 'rf' else np.logspace(-3, 0, 10)
    param_name = 'rf__n_estimators' if model_name == 'rf' else 'xgb__learning_rate'
    train_scores, test_scores = validation_curve(
        pipeline, X_train, y_train, param_name=param_name, param_range=param_range, cv=5, scoring='r2', n_jobs=-1
    )
    train_scores_mean = np.mean(train_scores, axis=1)
    test_scores_mean = np.mean(test_scores, axis=1)
